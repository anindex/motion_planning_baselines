...
from examples.pointmass_3d_rectangles_rrtstar import fixed_rectangles
...
if __name__ == "__main__":
    ...
    # -------------------------------- Environment ---------------------------------
    obst_primitives_l = fixed_rectangles(tensor_args=tensor_args)
    ...
    for traj in trajectories:
        ...
        ax.plot3D(traj_pos[:, 0], traj_pos[:, 1], traj_pos[:, 2], 'b-')
        ax.scatter3D(traj_pos[:, 0], traj_pos[:, 1], traj_pos[:, 2], color='b', s=10)
    start_state_np = to_numpy(q_start)
    goal_state_np = to_numpy(q_goal)
